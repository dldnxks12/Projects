...
for episode in range(MAX_EPISODES):
    ...
    while not done: # Stacking Experiences
        ...
        random_action = np.random.choice(action_index, 1)

        # 분포가 수렴하면서, random choice가 점점 무의미해질 것
        sample = random.random()
        if sample > 0.1:
            # Soft Max 분포에 따른 Action
            action = action_list[choice_action[0]].cpu().detach().numpy()
        else:
            # Soft Greedy Policy에 따른 Action
            action = action_list[random_action[0]].cpu().detach().numpy()
...
plt.savefig('TD3 check v2_random.png')

avg_history = np.array(avg_history)
np.save("./TD3_ensemble_model v2_random", avg_history)
np.save("./TD3_ensemble_model_v2_random_Softmax_result", softmax_recores)