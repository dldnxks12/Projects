...
try:
	while True:		
		...
		if qr_stop >= 4:
			...
			for _ in range(100):
				logger.info(f"START,{1},R,{end_right},D,{1},L,{end_left},D,{1},Z")
			sys.exit()
			continue
		...
		ROI_IMG     = cv2.inRange(ROI_IMG, (10, 100, 100), (30, 255, 255))	# Yellow range tuning...					
		...
		if (Init_distance == False):	# Check this block after distance init code finished ...		
			...
			if QR_bit == 1 and q < 5:
				q += 1
				print(f"Code : {decoded_direction} || q value : {q} || Wait for stable condition...")

				# UART
				if i % 5 == 0: 					
					logger.info(f"START,{Stop_bit},R,{end_right},D,{Right_direction},L,{end_left},D,{Left_direction},Z")
			...
			if len(decoded_QR) != 0: # QR CODE DETECTED !! 				
				...
				# QR Size Check 
				# if (d.rect[0] + d.rect[2] > 400) or (d.rect[1]+d.rect[3] > 400):
				if (d.rect[0] + d.rect[2] > 350) or (d.rect[1]+d.rect[3] > 350):
					QR_bit = 1
				else:
					QR_bit = 0 									
				...
				if QR_bit == 1:
					...
					elif decoded_direction == 'U': # Left 			
						Left_Wheel      = 100
						Right_Wheel     = 100
						Left_direction  = 0
						Right_direction = 1	
					
					elif decoded_direction == 'L': # U Turn 					
						Left_Wheel      = 100
						Right_Wheel     = 100
						Left_direction  = 0
						Right_direction = 1	
			...
			if (No_left_line_flag == 0) and (No_right_line_flag == 0) and (Left_Avg_points_temp != 0) and (Right_Avg_points_temp != 0) and QR_bit == 0:							
			...
			if (No_left_line_flag == 1) and (No_right_line_flag == 0) and (Right_Avg_points_temp != 0) and QR_bit == 0:			
			...
			if (No_left_line_flag == 0) and (No_right_line_flag == 1) and (Left_Avg_points_temp != 0) and QR_bit == 0:
